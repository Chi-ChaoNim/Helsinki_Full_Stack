...
const getWeather = async (lat, long) => {
    try {
        const request = axios.get(`${baseWeatherURL}&lat=${lat}&lon=${long}&appid=${weatherAPIKey}&units=metric`)
        return request.then(response => response.data)
    } catch (error) {
        console.error(error)
    }
...
const getGeoLocal = async (cityName, countryCode) => {
    try {
        const request = axios.get(`${baseGeoLocalURL}q=${cityName},${countryCode}&limit=1&appid=${weatherAPIKey}`)
        return request.then(response => response.data)
    } catch (error) {
        console.error(error)
    }
