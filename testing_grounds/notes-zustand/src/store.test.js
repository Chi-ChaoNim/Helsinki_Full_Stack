...
describe("useNoteActions", () => {
  it("initialize loads notes from service", async () => {
    //sets up mock value when the function is called
    const mockNotes = [{ id: 1, content: "Test", important: false }];
    noteService.getAll.mockResolvedValue(mockNotes);

    //binds the action functions to result
    const { result } = renderHook(() => useNoteActions());

    //tests the initialize function by calling it
    await act(async () => {
      await result.current.initialize();
    });

    //compares the returned result to the mock value given
...
describe("useNotes filtering", () => {
  ...
  it("return all notes with no filter", () => {
